...
dnspod_transfer() {
	...
	local A="$(date +%H%M%S) ERROR : [$__TMP] - terminating"
...
add_domain() {
	dnspod_transfer 1
	printf "%s\n" " $(date +%H%M%S) : add record success: [$([ "$__HOST" = @ ] || echo $__HOST.)$__DOMAIN],[IP:$__IP]" >> "$LOGFILE"
	return 0
}

update_domain() {
	dnspod_transfer 2
	printf "%s\n" " $(date +%H%M%S) : update record success: [$([ "$__HOST" = @ ] || echo $__HOST.)$__DOMAIN],[IP:$__IP],[TTL:$__TTL]" >> "$LOGFILE"
	return 0
}
...
describe_domain() {
	...
	if [ -z "$__TMP" ]; then
		printf "%s\n" " $(date +%H%M%S) : record not found: [$([ "$__HOST" = @ ] || echo $__HOST.)$__DOMAIN]" >> "$LOGFILE"
		ret=1
	else
		...
		if [ "$__RECIP" != "$__IP" ]; then
			...
			printf "%s\n" " $(date +%H%M%S) : record update needed: [remote:$__RECIP] [local:$__IP]" >> "$LOGFILE"
			ret=2
		fi
	fi
}
...
if [ "$ret" = 1 ]; then
	...
	printf "%s\n" " $(date +%H%M%S) : no update needed: [remote:$__RECIP] [local:$__IP]" >> "$LOGFILE"
fi
